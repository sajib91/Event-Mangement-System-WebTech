            ...
            if ($stmtApprovedEvents->rowCount() > 0) {
                while ($event = $stmtApprovedEvents->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    if (!empty($event['event_image'])) {
                        echo '<div class="mb-3">';
                        echo '<img src="' . htmlspecialchars($event['event_image'], ENT_QUOTES, 'UTF-8') . '" alt="Event Image" class="img-fluid rounded" style="max-height: 250px; width: 100%; object-fit: cover;">';
                        echo '</div>';
                    }
                    // Event details
